...
float circle(vec2 st, float radius, float blurriness) {
    vec2 dist = st;
    return 1.0 - smoothstep(radius-(radius*blurriness), radius+(radius*blurriness), dot(dist,dist)*4.0);
}
...
void main() {
    ...
    float shape = (uv.x + uv.y - 2.0 + progressHover * 2.7 + progress * 2.7) * 2.0;
    float offX = uv.x + uv.y;
    float offY = uv.y - uv.x;
    float n = snoise3(vec3(offX, offY, time) * 8.0) * .5;

    float grd = 0.1 * progressHover;
    float sqr = 100.0 * ((smoothstep(0.0, grd, uv.x) - smoothstep(1.0 - grd, 1.0, uv.x)) * 
                         (smoothstep(0.0, grd, uv.y) - smoothstep(1.0 - grd, 1.0, uv.y))) - 10.0;
    ...
    float pct = smoothstep(0.99, 1.0, n + shape);
...
interface NoiseCircleEffectProps {
    mainImageUrl: string;
    hoverImageUrl: string;
}

export default function NoiseCircleEffect({ mainImageUrl, hoverImageUrl }: NoiseCircleEffectProps) {
